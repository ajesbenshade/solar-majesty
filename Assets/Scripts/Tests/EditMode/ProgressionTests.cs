...
namespace SolarMajesty.Tests
{
    ...
    public class CampaignProgressTests
    {
        ...
        [Test]
        public void FreshEnsure_UnlocksEarth_AndIncludesAllFiveWorlds()
        {
            CampaignProgress.Ensure();
            Assert.AreEqual(CelestialBodyId.Earth, CampaignProgress.HighestUnlocked);
            Assert.AreEqual(CelestialBodyId.Earth, CampaignProgress.NewGameBody);
            Assert.IsTrue(CampaignProgress.IsUnlocked(CelestialBodyId.Earth));
            Assert.IsFalse(CampaignProgress.IsUnlocked(CelestialBodyId.Luna));
            Assert.IsFalse(CampaignProgress.IsUnlocked(CelestialBodyId.Mars));
            Assert.IsFalse(CampaignProgress.IsParked(CelestialBodyId.Earth));
            Assert.IsFalse(CampaignProgress.IsParked(CelestialBodyId.Europa));
        }

        [Test]
        public void MigrateHighest_PreservesOldEarthPrefs()
        {
            Assert.AreEqual(CelestialBodyId.Earth, CampaignProgress.MigrateHighest(CelestialBodyId.Earth));
            Assert.AreEqual(CelestialBodyId.Mars, CampaignProgress.MigrateHighest(CelestialBodyId.Mars));
        }

        [Test]
        public void ConqueringEachWorld_UnlocksTheNext_UntilEuropa()
        {
            CampaignProgress.Ensure();
            for (int i = (int)CelestialBodyId.Earth; i < (int)CelestialBodyId.Europa; i++)
            {
                var current = (CelestialBodyId)i;
                var next = (CelestialBodyId)(i + 1);
                Assert.AreEqual(next, CampaignProgress.NextAfter(current));
                Assert.IsFalse(CampaignProgress.IsUnlocked(next));
                CampaignProgress.UnlockNextFrom(current);
                Assert.IsTrue(CampaignProgress.IsUnlocked(next));
            }
            Assert.IsFalse(CampaignProgress.NextAfter(CelestialBodyId.Europa).HasValue);
        }

        [Test]
        public void ExplicitMarsDrop_PreservesEarlierWorldAccess()
        {
        ...
        [Test]
        public void ResetCampaign_ReturnsToEarth()
        {
        ...
        [Test]
        public void DebugUnlockAll_OpensAllFiveWorlds()
        {
            CampaignProgress.Ensure();
