...
namespace SolarMajesty
{
    ...
    /// extra solar / Defense are not a density gate — still campus skips that
    /// leftover pressure (Aaron 2026-09-07). Empty dirt is OK. Do not fill
    /// interior 4×4 sockets or crop the AABB. Play snap stays
    /// <see cref="PlayCampusOrthoSize"/>. FitStillOrtho never zooms inside that
    /// ortho. Does not flip spawnShowcaseColony. Does not reference Runtime
    /// ColonyLayout.
    /// </summary>
    public static class StillCampusDensity
    {
        ...
        public static bool TryNext(
            ...
            out Vector2Int origin,
            bool enforceIslandGap = true) =>
            TryNext(placer, commons, habFace, width, height, bounds, out origin, enforceIslandGap, 0);
        ...
        public static bool TryNext(
            ...
            out Vector2Int origin,
            int minGap) =>
            TryNext(placer, commons, habFace, width, height, bounds, out origin, true, minGap);
            ...
            int minGap)
        {
            ...
            var candidates = new List<Vector2Int>(160);
            ...
            int minGap = 0)
        {
            ...
            // Compact-majesty minGap still prefers islands; flush is last-resort.
            int prefer = Mathf.Max(minGap, MinYardGapCells);
            for (int gap = prefer; gap <= prefer + 3; gap++)
            {
            ...
            dest.Add(new Vector2Int(cx + cw + prefer, cy + ch + prefer));
            dest.Add(new Vector2Int(cx - width - prefer, cy + ch + prefer));
            dest.Add(new Vector2Int(cx + cw + prefer, cy - height - prefer));
            dest.Add(new Vector2Int(cx - width - prefer, cy - height - prefer));
