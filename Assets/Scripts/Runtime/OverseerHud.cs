...
namespace SolarMajesty
{
    ...
    public class OverseerHud : MonoBehaviour
    {
        ...
        private void DrawTitle()
        {
            ...
            GUI.Label(new Rect(c.x, c.y + 52f, c.width, 16f), "LUNA  →  MARS    ·    Earth / Belt / Europa parked", _wrap);
            GUI.Label(new Rect(c.x, c.y + 68f, c.width, 14f), ReplayRules.HudTag, _micro);
            GUI.Label(new Rect(c.x, c.y + 86f, c.width, 40f),
                "Luna is the first hour. Mars is the colony. Post bounties, let greedy robots choose. Three gates: clear dens, sustain, launch.",
                _wrap);
