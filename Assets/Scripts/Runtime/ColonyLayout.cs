...
namespace SolarMajesty
{
    ...
    public static class ColonyLayout
    {
        ...
        /// <summary>
        /// Ordered showcase pieces: path, local offset from CampusOrigin, yaw degrees, optional scale override.
        /// Layout reads as: pad/ship east → power south → habitat spine west → dome core → cmd/ops north.
        /// </summary>
        public static readonly ShowcasePiece[] Showcase =
        {
            // Core hub
            new ShowcasePiece("Buildings/SM_CommandDome_CentralHub", new Vector3(0f, 0f, 0f), 0f),

            // Habitat spine (west) — modules end-to-end along X with connectors
            new ShowcasePiece("Buildings/SM_HAB1_HabitatModule", new Vector3(-11f, 0f, 0f), 0f),
            new ShowcasePiece("Buildings/SM_ModularTubeConnector", new Vector3(-6.2f, 0f, 0f), 0f),
            new ShowcasePiece("Buildings/SM_LAB1_LaboratoryModule", new Vector3(-18.5f, 0f, 0f), 0f),
            new ShowcasePiece("Buildings/SM_ModularTubeConnector", new Vector3(-14.2f, 0f, 0f), 0f),

            // Command / ops (north)
            new ShowcasePiece("Buildings/SM_CMD1_CommandBuilding", new Vector3(-4f, 0f, 10f), 0f),
            new ShowcasePiece("Buildings/SM_OPS1_OperationsUnit", new Vector3(6f, 0f, 10f), 0f),

            // Power yard (south)
            new ShowcasePiece("Buildings/SM_PWR1_PowerNode", new Vector3(2f, 0f, -11f), 0f),
            new ShowcasePiece("Buildings/SM_PWR1_SolarArray", new Vector3(8.5f, 0f, -11f), 0f),

            // Landing complex (east) — pad under ship
            new ShowcasePiece("Environment/SM_LandingPad", new Vector3(16f, 0f, 0f), 0f, PadScale),
            new ShowcasePiece("Environment/SM_Starship_Placeholder", new Vector3(16f, 0f, 0f), 0f, ShipScale),
        };
        ...
        public readonly struct ShowcasePiece
        {
            ...
            public ShowcasePiece(string path, Vector3 localOffset, float yawDegrees, float scale = 0f)
            {
                ResourcesPath = path;
