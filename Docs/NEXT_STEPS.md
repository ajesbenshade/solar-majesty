...
1. Phase 4 visual target — **Weeks 1–4 continued are in**. Exit review is **blocked** ([PHASE_4_EXIT.md](Roadmap/PHASE_4_EXIT.md)). **Do not stamp Phase 4 complete. Do not start Phase 5 ship.** Human Game-tab still: Unity **Solar Majesty → Render → Capture Mars Still** (interactive editor, Game view Scale 1x, `spawnShowcaseColony` false). Writes `Docs/Roadmap/SM_Capture.png`. Judge vs [`SM_MarsCampus_SpacedOverseer_Concept.png`](Roadmap/SM_MarsCampus_SpacedOverseer_Concept.png) only. Capture now SyncFog + RefreshTubes + suit crossings after the still ortho snap.
