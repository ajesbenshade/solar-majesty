...
- [x] Phase 2 Weeks 7–9 — doctrine techs, 3 Secret Projects, Guild Hall, Harvester/Surveyor, new flags
- [x] Phase 2 Weeks 10–12 — body ecology, multi-campus rest/workshops, Phase 2 exit
- [x] Phase 3 Weeks 1–4 — roster to 10 classes + body-native fauna (Soil Creeper / Ash Hopper; 4–6 threats per world)
- [x] Phase 3 Weeks 5–7 start — Guild Hall class attraction; Climate Loom / Aegis Spire / Deep Archive wonders; body-tinted greybox hulls
...
4. **Week 4** — Juice *(done)*
5. **Week 5–6** — Demo package *(packaging done; playtest / exit review open)*
...
1. Phase 4 visual target — **Weeks 1–4 continued are in**. Exit review is **blocked** ([PHASE_4_EXIT.md](Roadmap/PHASE_4_EXIT.md)): still5 (`SM_MarsCampaign_PlayModeCampusStill5.png`) showed unused orange Commons **port rings** (hull-drum `CommonsPort_*`, not just `CommonsStub` / `DockSleeve`). v4 was orange box airlock + unused ribbed stub. Latest Game-tab is still6 (empty Sol 1). Code now starts hull ports hidden and RefreshTubes enables docked faces only — **not** in those PNGs. Next human step: re-enter Play Mode on Mars, place Commons + airlock + HAB, look for a **white paneled square hub**, **short white tubes + one orange collar on docked faces only**, **no unused CommonsPort rings**, **readable white hulls**, camera snapping to campus ortho **10**. Then a fresh Game-tab still. **Do not stamp Phase 4 complete. Do not start Phase 5.** Square Lego airlocks stay; tubes/domes/cranes/kits/turrets/solar are dressing. Mockup squad bars are HUD chrome only. Do not add click-to-move. Defense stays the tracked Guardian.
