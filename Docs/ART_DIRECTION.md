...
3. **Distant haze** toward the horizon — Mars exp2 fog 0.014 plus `MarsHazeRoot` cards (campus hulls stay unwashed).
4. **Polyhedron / geodesic Commons** — faceted `SM_CommonsGeodesic` + carbon `CommonsLattice` ribs, not a smooth hemisphere-only.
5. Colonists crossing open ground **read as spacesuited** (`CampusDress_Colonists` still / play dressing). Do **not** invent new `FlagTypes` or rewrite `SpecialistBrain`.
