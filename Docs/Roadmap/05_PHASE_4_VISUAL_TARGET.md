...
**Week 4 continued (this slice):** Campus Game-tab stills v1–**v6** plus a look-kit hide pass for still5 unused Commons hull-drum ports (`CommonsPort_*` / aliases). still16 signed the remaining look fail as a **dark rectangular airlock box** (wrap `Dress_HubDoor`). Hub is a smaller white paneled square; orange collars on docked faces only; no fourth `CampusTubeRoot`. still18 signed the density gate (pad / solar / extractors). still19 leftover=skip-frame left empty-dirt framing. CaptureStill now stamps leftover Workshop / Inn / wonder when they CanFit, plus extra HAB / solar / defense cues; still snap fits the packed AABB (play ortho stays 10). still20 signed leftover=workshop + extraHab but **W2 cut / narrative text covered the campus** — shutter hold now suppresses those modals. Empty-Sol-1 still6 stays archived. Honest notes below — **not** a Phase 4 exit. A new Game-tab still is still required.
