...
- [x] HAB kit: boxy beige/tan hull + roof solar (locked Play-mode still). Square airlocks still attach. **Not** HAB-1 graphite-rim cylinder. Commons geodesic restyle is PR 41.
...
- Tube campus on the **square** Lego grid: **white cube** airlock hubs with **dark square windows**, **orange ribbed** Commons-to-HAB collars with **orange torus rings** on docked faces only; unused cardinal sleeves stay hidden
...
- HAB as a **boxy beige/tan module** with roof solar (locked Play-mode still — not HAB-1 graphite-rim cylinder, not a Commons dome)
- Colony Commons as the large central **command-dome citadel** (player-facing **COMMONS**, not Palace / not mockup “Command Center” label) with a frequency-3 cream geodesic facet lattice over a dark geodesic undershell, orange equatorial / cupola bands, and drum meridians. CaptureStill landmark yards keep 4 empty cells off the dome.
...
- Landing pad: extra yellow tier ring under the Starship stack; parked rocket is the **procedural** cream stack with **two thick carbon bands** (placeholder FBX skipped on the pad — cool-white 33 % flank). Tiny hull fill for a soft terminator. LaunchSite still uses `SM_Starship_Placeholder`.

**Still greybox / not an exit stamp**
- Campus Game-tab still v4 (`SM_MarsCampaign_PlayModeCampusStill4.png`): geodesic Commons + HAB cylinder, **no grey hex pads**, **no idle hopper chip**, hulls whiter than v3 — but **orange box airlock**, unused orange ribbed stub, square hub not readable. still5 (`SM_MarsCampaign_PlayModeCampusStill5.png`): unused Commons cardinal still shows an orange **port ring**. **Locked HAB still (Aaron):** boxy beige/tan hull + roof solar + orange ribbed Commons-HAB collar + white cube airlock with dark square windows. Graphite-rim HAB-1 is **rejected**. Commons geodesic restyle stays PR 41. still21 leftover=inn+wonder extraHab/solar/defense is **not** a packed-density fail. Bake-off PRs **#26 / #27 / #28** are **not** EXIT. Empty dirt is OK. Needs a **fresh** spaced Game-tab look vs the locked concept — do not stamp v4/v5/still21/bake-off.
