...
**Week 4 continued (this slice):** Parked pad Starship skips `SM_Starship_Placeholder` (cool-white 33 % shaded flank). Procedural stack is warm cream with two thick carbon bands (~30 % / ~58 % height) and a tiny hull fill so the terminator can sit near 55 % of lit — matching [`SM_MarsCampus_SpacedOverseer_Concept.png`](SM_MarsCampus_SpacedOverseer_Concept.png). Orange hull stripes retired. LaunchSite still uses the placeholder craft. Does **not** retune geodesic Commons or 4-cell island yards (PR #41). Needs a new GD Game-tab Capture before re-judge. Honest notes below — **not** a Phase 4 exit. **Do not stamp exit.**
