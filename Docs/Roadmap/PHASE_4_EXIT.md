...
- **Campus Game-tab still (Aaron 2026-09-07 vs concept)** — latest archived Game-tab is still6 (empty Sol 1). Bake-off #26/#27/#28 Captures are **not** EXIT. still21 leftover=inn+wonder extraHab/solar/defense is **not** a density gate. This pass is **code only**: parked pad Starship is the procedural cream stack with two thick carbon bands + hull fill (placeholder FBX skipped on the pad). CaptureStill prefers play ortho 10; leftover kits do **not** stamp; **RefreshTubes no longer stamps `CampusDress_TubeRuns` (`SpawnTubeRuns` removed)**; interior dirt is not force-filled. `StillCaptureHold` stays. **Needs a new Game-tab still from GD after merge. Do not stamp exit.**
