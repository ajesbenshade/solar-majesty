...
**Look pass 2 (still blocked).** Ground grit / seamless regolith tile, hull surface on the pad + extractor + ship kits, neutral lower-hull grime, de-neoned solar, gravel litter, softer aprons, and canvas awnings landed as code and bake changes. **None of it has been seen in engine** — no Unity on the Cloud VM, so there is no still and **no judge score**. Delta table and the open items are in [`DREAM_LOOP_MARS_LOOK.md`](DREAM_LOOP_MARS_LOOK.md). One item needs an Aaron decision, not a fix: the concept's sky / horizon band cannot appear at the pinned orthographic still camera, so either the layout-language match is accepted or a second perspective hero shot is authorised. **Exit stays blocked.**
