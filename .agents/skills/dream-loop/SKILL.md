...
**Solar Majesty lock:** Phase 4 look is Aaron’s **2026-09-07 spaced ITS campus** vs `Docs/Roadmap/SM_MarsCampus_SpacedOverseer_Concept.png` — no interconnect tube webs, empty red dirt OK, distant haze, spacesuited crossings. **Commons is the locked command-dome citadel** (`HeroBuildingKits.BuildCommons`) — do not restore geodesic lattice. Bake-off PRs #26/#27/#28 are not EXIT. Do not chase packed-city density. Working files go in `.dream-loop/` (gitignored). Team-facing concept lives under `Docs/Roadmap/`.
...
Put working context/files in `.dream-loop` and gitignore this folder (unless told otherwise by the user).
...
**Solar Majesty extra gate:** reject any concept that packs buildings into every dirt patch. Required: spacious campus pads, negative space, overseer strategy view.
